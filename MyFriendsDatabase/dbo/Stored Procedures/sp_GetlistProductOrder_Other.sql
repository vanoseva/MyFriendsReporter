CREATE Procedure [dbo].[sp_GetlistProductOrder_Other](@s_NumberID nvarchar(50),@s_Store_ID nvarchar(50))    
as    
 ...
 Begin    
  if @s_NumberID=''    
   select pr.s_store_id, pr.f_curinstock, ls.*    
            ...
            Where pr.s_store_id=@s_Store_ID and pr.f_curinstock>0  and ls.b_Visible=1  and (ls.b_IsMaterial=1 or ls.b_IsEndProduct=1)
            Order by ls.s_Name asc    
  else    
   select pr.s_store_id, (pr.f_curinstock + isnull(t.Qty,0)) as  f_curinstock, ls.*    
             from ls_products ls join pr_product_store pr on ls.s_id=pr.s_product_id and pr.s_store_id=@s_Store_ID      
                 
             left join (Select s_Product_ID,sum(f_Convert*f_Quantity) as Qty from V_OrderDetail_Other where s_OrderID=@s_NumberID Group by s_Product_ID) as t     
             On pr.s_Product_ID=t.s_Product_ID    
    
             Where  (pr.f_curinstock + isnull(t.Qty,0))>0 and ls.b_Visible=1 and (ls.b_IsMaterial=1 or ls.b_IsEndProduct=1)   
             Order by ls.s_Name asc    
 end    
 Else    
 Begin    
